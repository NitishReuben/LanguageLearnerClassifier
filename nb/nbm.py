...
class NB_model():
    ...
    def fit(self, train_data, train_label, vocab, if_use_smooth=True):
        ...
        for c in classes:
            ...
            self.Pr_dict[c] = df_class.groupby(
                'wordIdx')['count'].sum().to_dict()
            self.zero_word_dict[c] = 1 / (total + self.num_vocab)
            for k in self.Pr_dict[c]:
                self.Pr_dict[c][k] = (
                    self.Pr_dict[c][k] + 1) / (total + self.num_vocab)
            ...
            for classIdx in range(1, self.num_classes+1):
                ...
                for w in new_dict[docIdx]:
                    score_dict[classIdx] += np.log(
                        self.Pr_dict[classIdx].get(w, self.zero_word_dict[classIdx]))
